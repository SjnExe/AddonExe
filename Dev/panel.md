...
| `helpfulLinks` | `§9Helpful Links` | `textures/items/chain` | `functionCall` | `showHelpfulLinks` |
...
- **`showRules` action**: This is a `functionCall` that triggers the `showRules` function in `uiManager.js`. This function creates a new `ActionFormData` on the fly, populates its body with the rules from `config.js`, and shows it to the player. If the player is an admin, an "Edit Rules" button is shown, which opens the `rulesManagementPanel`.
- **`showHelpfulLinks` action**: Similar to `showRules`, this `functionCall` triggers `showHelpfulLinks` in `uiManager.js`. It dynamically creates a panel whose body is populated with link titles and URLs from `config.js`. For admins, it includes an "Edit Links" button that opens the `helpfulLinksManagementPanel`, allowing for full in-game configuration.
