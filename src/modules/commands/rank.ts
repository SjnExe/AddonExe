...
const rankCommand: CustomCommand = {
    ...
    execute: (player, args) => {
        ...
        if (!action || (action as string).toLowerCase() === 'list') {
        ...
        const actionLC = (action as string).toLowerCase();
        ...
        if ((rankId as string).toLowerCase() === 'owner' || (rankId as string).toLowerCase() === 'member') {
            ...
            if (player instanceof mc.Player) {playSound(player, 'note.bass');}
            return;
        }
        const rankDef = rankManager.getRankById((rankId as string).toLowerCase());
        if (!rankDef) {
