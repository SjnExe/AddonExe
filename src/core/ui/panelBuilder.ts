...
export async function getPanelItems(player: mc.Player, panelId: string, context: UIContext): Promise<PanelItem[]> {
    ...
    if (panelId === 'tpaSettingsPanel') {
        addBack('gameplayPanel'); // Correct Back target
    ...
    if (panelId === 'rankManagementPanel') {
        ...
        items.push(...staticItems); // Handles Back (via parentPanelId) and static buttons

        const ranks = rankManager.getAllRanks();
        ranks.forEach(rank => {
             items.push({
                 id: rank.id,
                 text: `${rank.chatFormatting.nameColor}${rank.name}`,
                 permissionLevel: 1024,
                 actionType: 'openPanel',
    ...
    if (panelId === 'playerListPanel' || panelId === 'playerManagementPanel') {
        if (panelId === 'playerListPanel') addBack('gameplayPanel'); // Correct Back target
        else addBack('adminPanel'); // Correct Back target
    ...
    if (panelId === 'rulesManagementPanel') {
        addBack('infoPanel'); // Correct Back target
    ...
    if (panelId === 'helpfulLinkActionPanel') {
        addBack('helpfulLinksManagementPanel');
        if (permissionLevel <= 1) {
            items.push({ id: 'edit', text: 'Edit', icon: 'textures/ui/icon_setting', permissionLevel: 1, actionType: 'functionCall', actionValue: 'editHelpfulLink' });
            items.push({ id: 'moveUp', text: 'Move Up', icon: 'textures/gui/controls/up', permissionLevel: 1, actionType: 'functionCall', actionValue: 'moveHelpfulLinkUp' });
            items.push({ id: 'moveDown', text: 'Move Down', icon: 'textures/gui/controls/down', permissionLevel: 1, actionType: 'functionCall', actionValue: 'moveHelpfulLinkDown' });
            items.push({ id: 'delete', text: 'Delete', icon: 'textures/ui/trash', permissionLevel: 1, actionType: 'functionCall', actionValue: 'deleteHelpfulLink' });
        }
        return items;
    }
...
export async function buildPanelForm(
    ...
    try {
        ...
        if (panelId.startsWith('configSubCategoryPanel_')) {
            const catId = panelId.replace('configSubCategoryPanel_', '');
            title = catId.charAt(0).toUpperCase() + catId.slice(1) + ' Configuration';
        }

        // Resolve placeholders in title
        if (title.includes('{playerName}') && context.selectedItemId) {
             const pData = getPlayer(context.selectedItemId as string) || loadPlayerData(context.selectedItemId as string);
...
async function buildModalForm(player: mc.Player, panelId: string, context: UIContext): Promise<ModalFormData | null> {
    ...
    if (panelId === 'editRankPanel') {
        const rankId = (context.rankId || context.id) as string | undefined;
    ...
    if (panelId === 'editXrayOrePanel') {
        ...
        const oreIndex = context.oreIndex !== undefined ? Number(context.oreIndex) : Number(context.id);
