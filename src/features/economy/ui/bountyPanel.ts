...
export async function showBountyListPanel(player: mc.Player, context: UIContext = {}): Promise<void> {
...
export async function showBountyPlayer(player: mc.Player, context: UIContext): Promise<void> {
    ...
    const amountStr = res.formValues!.amount;
...
export async function showRemovePlayerBounty(player: mc.Player, context: UIContext): Promise<void> {
    const targetId = context.targetPlayerId as string;
    if (!isNonEmptyString(targetId)) return;
