...
try {
    ...
    const runTests = await $`bun test --isolate --parallel`.nothrow().quiet();

    if (runTests.exitCode !== 0) {
        console.error(`=== Unit Tests Failed (Exit: ${runTests.exitCode}) ===\n${runTests.stdout}${runTests.stderr}`);
